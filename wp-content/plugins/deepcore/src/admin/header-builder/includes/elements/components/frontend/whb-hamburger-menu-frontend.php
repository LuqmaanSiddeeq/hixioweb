...
function whb_hamburger_menu( $atts, $uniqid, $once_run_flag ) {
	...
	// render
	$out .= '
	<div class="whb-element whb-icon-wrap whb-hamburger-menu ' . esc_attr( $extra_class ) . ' ' . $hamburger_type . ' ' . $dark_wrap . ' ' . $hamburger_icon . '" id="whb-hamburger-menu-' . esc_attr( $uniqid ) . '" data-push-content=' . esc_attr( $push_content ) . '>
		...
		if ( $socials == 'true' ) :
			ob_start(); ?>
						<div class="hamburger-social-icons">
							<?php include DEEP_SRC_DIR . 'templates/social.php'; ?>
						</div>
