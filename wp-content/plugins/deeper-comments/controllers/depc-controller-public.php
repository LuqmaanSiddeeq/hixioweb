...
class Depc_Controller_Public extends Depc_Controller {
	...
	protected function register_hook_callbacks() {
		...
		$this->recaptcha['recaptcha_version']  = Depc_Core::get_option( 'dc_recaptcha_version', 'Recaptcha', '3' );//TODO: Get From Settings
		...
		if ( is_singular() ) {
			...
			if ( comments_open() ) {
				...
				if ( $this->recaptcha['recaptcha'] == 'google' ) {
					$captcha_version = $this->recaptcha['recaptcha_version'];
					...
					if( 2 == $captcha_version ){
						...
						wp_enqueue_script(
							...
							'https://www.google.com/recaptcha/api.js?hl='.str_replace('_', '-', $locale),
							array( 'jquery' ),
							Depc_Core::DEPC_VERSION,
							false
						);
						...
						$google_site_key = $this->recaptcha['recptcha_gsitekey'];
						wp_enqueue_script(
							Depc_Core::DEPC_ID . 'recaptcha',
	...
	public function loclize(){
		...
		wp_localize_script( Depc_Core::DEPC_ID , 'dpr', array(
			...
			'recaptcha_version'		=> $this->recaptcha['recaptcha_version'],
