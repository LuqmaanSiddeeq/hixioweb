...
function deep_theme_setup() {
	...
	/**
	 * Prevent modern events calendar redirection.
	 */
...
/**
 * Add deep options to admin menu
 *
 * @since 1.0.0
 */
function deep_admin_bar_link() {
	...
	$menu_visiblity        = array(
		...
		'help' => ! empty( $deep_options['deep_theme_menus']['help'] ) ? $deep_options['deep_theme_menus']['help'] : '0',
	...
	$theme_name 		   = ( isset( $custom_name ) && $custom_name != null ) ? $custom_name : 'Deep';
	...
	$wp_admin_bar->add_menu(
		array(
			...
			'parent' => 'wn_main_menu_item',
			'title'  => 'Options',
			'href'   => $admin_url . 'wn-admin-options',
		)
	);
}
...
/**
 * Fix WPBakery Page Builder extra p tag.
 *
 * @since 1.0.0
 */
...
if ( ! function_exists( 'remove_redux_themeoption_from_adminbar' ) ) :
	function remove_redux_themeoption_from_adminbar( $wp_admin_bar ) {
		$wp_admin_bar->remove_node( 'wn-admin-options' );
	}
endif;
