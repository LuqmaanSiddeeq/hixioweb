...
$deep_footer_dynamic_copyright	= $deep_options['deep_footer_dynamic_copyright'] == '1' ? '<p class="copyright">' . esc_html__( '© ', 'deep' ) . date( 'Y' ) . ' ' . '<a href="' . home_url() . '">' . get_bloginfo( 'name') . '</a>' . esc_html__( ' - All Rights Reserved', 'deep' ) . '</p>' : '' ;
$deep_footer_custom_copyright	= $deep_options['deep_footer_dynamic_copyright'] == '0' && isset( $deep_options['deep_footer_custom_copyright'] ) ? wp_kses( $deep_options['deep_footer_custom_copyright'], deep_kses() ) : '' ;
$w_fb_menu = $w_fb_logo = '';
		...
		<div class="<?php echo esc_attr( $fb_column ); ?>">
			<div class="footer-navi">
				<?php switch ( $w_fbl_type ) {
					case 1: echo ' ';
					break;
					case 2: echo wp_kses( $w_fb_logo, wp_kses_allowed_html( 'post' ) ) ;
					break;
					case 3:	echo wp_kses( $w_fb_menu, wp_kses_allowed_html( 'post' ) ) ;
					break;
					case 4:	echo wp_kses( $deep_footer_copyright, wp_kses_allowed_html( 'post' ) ) ;
					break;
					case 5: include DEEP_SRC_DIR . 'templates/social.php';
					break;
				} ?>
			</div>
		</div>
