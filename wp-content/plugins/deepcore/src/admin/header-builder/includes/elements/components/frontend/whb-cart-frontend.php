...
function whb_cart( $atts, $uniqid, $once_run_flag ) {
	...
	if ( WHB_Helper::is_frontend_builder() ) {
		...
		$cart_count = $woocommerce ? $woocommerce->cart->cart_contents_count : esc_html__( '0', 'deep' );
	...
	$open_icon_id = $open_cart_type == 'dropdown' ? 'wn-cart-modal-icon' : '';
	// render
	$out .= '
